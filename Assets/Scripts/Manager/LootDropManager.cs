...
namespace Manager
{
    ...
    public class LootDropManager : SingletonMono<LootDropManager>
    {
        ...
        [Header("DropItem 专用 Physics Layer")]
        ...
        /// 由 DropOnDeath 调用。
        /// </summary>
        public void SpawnWorldDrop(ItemConfig config, int count, Vector3 position)
        {
            ...
            var entity = _world.Create(
                ...
                new ViewReference(go)   // ViewReference 只存 GO，解耦 ICharacterView
            );
            ...
            // 手动拾取物注册到 InteractManager
            if (!config.AutoPickup)
                InteractManager.Instance?.RegisterDropItem(item);

            item.ApplyBounceForce();
        }
        ...
        public void Collect(WorldDropItem item)
        {
            ...
            InventoryManager.AddItem(item.Config, item.Count);
        ...
        private void DetectAutoPickup()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var dropItem = col.GetComponent<WorldDropItem>();
