...
public class PlayerLedgeClimbState : PlayerMovementState
{
    ...
    public override void OnEnter()
    {
        ...
        targetPoint = reusableData.hit.point + Vector3.up * HangWallData.hightAndForwardOffSet.x +
                      reusableData.hit.normal * (HangWallData.hightAndForwardOffSet.y);
        base.OnEnter();
    }
    ...
    private void OnMove(InputAction.CallbackContext context)
    {
        ...
        if (angle < 45 && angle >= -45)
        {
            ...
            ClimbTargetMatchInfo climbTargetMatchInfo =
                new ClimbTargetMatchInfo(reusableData.hit.point + Vector3.up * 0.25f);
            reusableLogic.SetClimbTarget_Y_Task(animancerState, ref climbTargetMatchInfo, 0.6f, 1);
            animancerState.Events(player)
                .SetCallback(playerSO.playerParameterData.moveInterruptEvent, OnInputInterruption);
    ...
    public override void OnUpdate()
    {
        ...
        if (isInitMatchTargeting) //抓上去匹配或者取消攀爬时的重新匹配
        {
            player.transform.position =
                Vector3.SmoothDamp(player.transform.position, targetPoint, ref matchSpeed, 0.05f, 200);
        ...
        {
            player.transform.position =
                Vector3.SmoothDamp(player.transform.position, targetPoint, ref matchSpeed, 0.08f, 80);
        ...
        else //爬匹配
        {
            ...
            if (Physics.Raycast(startDetectionPos, player.transform.forward, out var ray, 1.0f, player.whatIsGround))
            {
                ...
                if (Physics.Raycast(startDetectionPos + Vector3.up * maxError, player.transform.forward,
                        out var hitInfo, 0.5f, player.whatIsGround) && !isHandingRotation.Value)
                {
                    reusableData.hit = hitInfo;
                    Debug.DrawLine(startDetectionPos + Vector3.up * maxError,
                        startDetectionPos + Vector3.up * maxError + player.transform.forward * 1f, Color.blue, 0.05f);
                }
                else
                {
                ...
                Vector3 target = reusableData.hit.point + (Vector3.up * HangWallData.hightAndForwardOffSet.x) +
                                 reusableData.hit.normal * (HangWallData.hightAndForwardOffSet.y);
                ...
                Debug.DrawLine(startDetectionPos, startDetectionPos + player.transform.forward * 1.0f, Color.red,
                    0.05f);
            }
            else
            {
                if (Physics.Raycast(startDetectionPos + Vector3.down * maxError, player.transform.forward,
                        out var hitInfo, 0.5f, player.whatIsGround) && !isHandingRotation.Value)
                {
                    reusableData.hit = hitInfo;
                    ...
                    if (Vector3.Distance(player.transform.position, target) > 0.04f)
                    {
                        ...
                        Debug.DrawLine(startDetectionPos + Vector3.down * maxError,
                            startDetectionPos + Vector3.up * maxError + player.transform.forward * 0.8f, Color.blue,
                            0.05f);
                    }
                }
                else
    ...
    private void HandRotaion(bool noWall)
    {
        ...
        if (noWall)
        {
            ...
            Vector3 detectionDir = reusableData.rotationValueParameter.CurrentValue >= 0
