...
// Modified for Unity 2021.3 compatibility by adding UNITY_2022_2_OR_NEWER preprocessor directives.
...
#else
            if (TrySwapTexture(_SpriteMap, _Texture, null, ref sprite))
#endif
...
#else
                // Unity 2021.3 版本使用 8 参数的 Sprite.Create 重载（不支持 secondaryTextures）
                otherSprite = Sprite.Create(texture,
                    sprite.rect, pivot, sprite.pixelsPerUnit,
...
#else
        /// <summary>Secondary textures are not supported in Unity versions prior to 2022.2.</summary>
        public static SecondarySpriteTexture[] GetSecondaryTexturesCached(Sprite sprite)
        {
            return System.Array.Empty<SecondarySpriteTexture>();
        }
