...
public class PlayerReusableLogic
{
    ...
    public void InitIdleState()
    {
        ...
        if (reusableData.standIdleMixerState != null &&
            (reusableData.standIdleList.Count != reusableData.standIdleMixerState.ChildCount ||
        ...
        if (reusableData.crouchIdleMixerState != null &&
            (reusableData.crouchIdleList.Count != reusableData.crouchIdleMixerState.ChildCount ||
    ...
    public void OnJump()
    {
        ...
        RaycastHit hit = GetWallHight(player.transform.forward, canClimbMinHeight, canClimbMaxHight, detectionDistance,
            ref vaultHeight, ref obstructHeight, detectionSamplingCount);
        float angle = Vector3.Angle(-player.transform.forward, hit.normal);
        RayDebug.Log("angle:" + angle);
        RayDebug.Log("hit:" + hit.point);
        RayDebug.Log("vaultHeight:" + vaultHeight);
    ...
    /// <returns></returns>
    private RaycastHit GetWallHight(Vector3 targetDir, float startDetectionHight, float maxDetectionHight,
        float detectionLength, ref float vaultHight, ref float obstructHeight, int detectionSamplingCount)
    {
    ...
    /// </summary>
    public void ClimbTargetMatch(AnimancerState animancerState, ref ClimbTargetMatchInfo climbTargetMatchInfo,
        float startNormalizedTime, float endNormalizedTime)
    {
    ...
    /// </summary>
    public void ClimbTargetMatch_Y(AnimancerState animancerState, ref ClimbTargetMatchInfo climbTargetMatchInfo,
        float startNormalizedTime, float endNormalizedTime)
    {
        float currentTime = animancerState.NormalizedTime;
        if (!climbTargetMatchInfo.setTargetMatchInitPos && animator.States.Current.NormalizedTime > startNormalizedTime)
        ...
        if (currentTime > startNormalizedTime && currentTime < endNormalizedTime)
        {
            ...
            Vector3 targetPos = new Vector3(player.transform.position.x,
                Mathf.Lerp(climbTargetMatchInfo.InitPos.y, climbTargetMatchInfo.TargetPos.y, t),
                player.transform.position.z);
            player.transform.position = targetPos;
        }
    }

    public void SetClimbTarget_Y_Task(AnimancerState animancerState, ref ClimbTargetMatchInfo climbTargetMatchInfo,
        float startNormalizedTime, float endNormalizedTime)
    {
    ...
    public void SetAnimationMotionCompensationTask(Vector3 compensationMovement, AnimancerState targetAnimation,
        float startNormalTime = 0, float endNormalizedTime = 1)
    {
        animationTask = () =>
        {
            ...
            if (targetAnimation.NormalizedTime >= startNormalTime &&
                targetAnimation.NormalizedTime <= endNormalizedTime)
            {
                //计算目标时间内每帧的补偿量
                ...
                RayDebug.Log("正在补偿位移：" + player.animatorDeltaPositionOffset);
            }
            else
            {
