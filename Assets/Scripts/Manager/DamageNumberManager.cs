...
namespace Manager
{
    ...
    /// </summary>
    public class DamageNumberManager : MonoBehaviour, Battle.IDamageNumberService
    {
        public static DamageNumberManager Instance { get; private set; }

        [Title("飘字预制体")] [LabelText("普通伤害"), Required] [SerializeField]
        private DamageNumber normalPrefab;

        [LabelText("暴击伤害（金黄/放大）")] [SerializeField]
        private DamageNumber critPrefab;

        [LabelText("治疗（绿色）")] [SerializeField] private DamageNumber healPrefab;

        [Title("显示参数")] [LabelText("随机 X 偏移范围"), Range(0f, 1f)] [SerializeField]
        private float randomXOffset = 0.3f;

        [LabelText("头顶高度额外偏移")] [SerializeField]
