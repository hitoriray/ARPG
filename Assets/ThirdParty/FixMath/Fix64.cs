...
namespace FixMath
{
	...
	public partial struct FP : IEquatable<FP>, IComparable<FP>
	{
		...
		const long HUNDRED = 100L << FRACTIONAL_PLACES;
		...
		const int LUT_SIZE = (int)(PI_OVER_2 >> 23);
		...
		public void Abs()
		{
			if (_serializedValue < 0)
				_serializedValue = -_serializedValue;
		}
		...
		public static FP Sqrt(FP x)
		{
			...
			if (xl < 0)
			{
				...
				//throw new ArgumentOutOfRangeException("Negative value passed to Sqrt? Maybe num to big", "x");
			}
		...
		{
			return (int)(value._serializedValue >> FRACTIONAL_PLACES);
		}

		public static implicit operator FP(string input)
		{
			string value = string.Copy(input);
			...
			int rmCnt = 0;
			while (raw > int.MaxValue || raw < int.MinValue)
			{
				rmCnt++;
				ten /= 10;
				unsafe
				...
				raw = long.Parse(value);
			}
			raw = raw << FRACTIONAL_PLACES;
			if (ten != 0)
				raw = raw / ten;
			return new FP(raw);
		}

		public static FP Parse(string text)
		{
			return (FP)text;
		}
        ...
        public static FP FromString(string input)
        {
            return (FP)input;
        }
