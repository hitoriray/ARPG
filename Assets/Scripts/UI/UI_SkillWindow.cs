...
namespace UI
{
    [UIWindowData(typeof(UI_SkillLearnWindow), true, nameof(UI_SkillWindow), 2)]
    public class UI_SkillWindow : UI_WindowBase
    {
        ...
        public void Show(SkillLearnedDatas skillLearnedDatas)
        {
            ...
            foreach (var item in skillLearnedDatas.SkillLearnedDataDict.Dictionary)
            {
                ...
                {
                    releaseSlotList[releaseSkillIndex++].Show(item.Value, skillConfig, true);
                }
                else
                {
                    passiveSlotList[passiveSkillIndex++].Show(item.Value, skillConfig, false);
                }
            }

            for (int i = releaseSkillIndex; i < slotCount; i++)
            {
                releaseSlotList[i].Show(null, null, true);
            }
            for (int i = passiveSkillIndex; i < slotCount; i++)
            {
                passiveSlotList[i].Show(null, null, true);
            }
        }
    }
}