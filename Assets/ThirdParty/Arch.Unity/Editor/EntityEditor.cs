...
namespace Arch.Unity.Editor
{
    ...
    public sealed class EntityEditor : UnityEditor.Editor
    {
        ...
        static readonly HashSet<object> visitedObjects = new HashSet<object>(); // 添加循环引用检测
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!proxy.entityReference.IsAlive())
            {
                return;
            }
        ...
        static void DrawMembers(object target, string rootPath, int depth)
        {
            // 添加深度检查，防止递归过深
            if (depth > 2) {
                EditorGUILayout.LabelField("Max depth exceeded");
                Debug.LogError("Max depth exceeded");
                return;
            }
            ...
            // 循环引用检测
            if (!visitedObjects.Add(target))
            {
                EditorGUILayout.LabelField("Circular Reference Detected");
                return;
            }
            ...
            try
            {
                FieldInfo[] fieldInfos = target.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public);
                foreach (FieldInfo fieldInfo in fieldInfos)
                {
                    object value = null;
                    try
                    {
                        // 添加更全面的异常处理，防止GetValue调用时崩溃
                        value = fieldInfo.GetValue(target);
                    }
                    catch (Exception ex)
                    {
                        EditorGUILayout.LabelField(ObjectNames.NicifyVariableName(fieldInfo.Name), $"Error: {ex.GetType().Name}");
                        continue;
                    }
                    ...
                    switch (value)
                    {
                        ...
                        case UnityEngine.Object fieldValue:
                            ...
                            break;
                        case FP fieldValue:
                            EditorGUILayout.FloatField(label, fieldValue.AsFloat());
                            break;
                        case TSBBox fieldValue:
                            EditorGUILayout.TextField(label, fieldValue.ToString());
                            break;
                        case TSBBox2D fieldValue:
                            EditorGUILayout.TextField(label, fieldValue.ToString());
                            break;
                        case TSQuaternion fieldValue:
                            EditorGUILayout.Vector3Field(label, fieldValue.eulerAngles);
                            break;
                        case TSQuaternion2D fieldValue:
                            EditorGUILayout.Vector3Field(label, fieldValue.eulerAngles);
                            break;
                        case TSVector2 fieldValue:
                            EditorGUILayout.Vector2Field(label, fieldValue);
                            break;
                        case TSVector3 fieldValue:
                            EditorGUILayout.Vector3Field(label, fieldValue);
                            break;
            ...
            {
                Debug.LogError("DrawMembers error:" + e);
            }
            finally
            {
                // 从已访问集合中移除当前对象
